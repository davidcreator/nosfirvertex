...
<section class="card">
    ...
    <p class="muted">Gerencie parâmetros globais e a área de doações do catálogo.</p>

    <form method="post" action="<?= e(base_url('admin/index.php?route=settings')) ?>">
        <input type="hidden" name="csrf_token" value="<?= e($csrf_token) ?>">

        <section class="card" style="margin-bottom:12px;">
            <h2>Área de doações</h2>
            <p class="muted">Campos vazios não serão exibidos para o usuário final.</p>

            <div class="grid cols-3">
                <?php $enabledValue = $readSetting('donation_enabled'); ?>
                <div class="card" style="margin-bottom:0;">
                    <label for="donation_enabled">Doações ativas</label>
                    <select id="donation_enabled" name="donation_enabled">
                        <option value="1"<?= $enabledValue !== '0' ? ' selected' : '' ?>>Sim</option>
                        <option value="0"<?= $enabledValue === '0' ? ' selected' : '' ?>>Não</option>
                    </select>
                </div>

                <div class="card" style="margin-bottom:0;">
                    <label for="donation_title">Título</label>
                    <input id="donation_title" name="donation_title" value="<?= e($readSetting('donation_title')) ?>">
                </div>

                <div class="card" style="margin-bottom:0;">
                    <label for="donation_pix_beneficiary">Nome beneficiário</label>
                    <input id="donation_pix_beneficiary" name="donation_pix_beneficiary" value="<?= e($readSetting('donation_pix_beneficiary')) ?>">
                </div>

                <div class="card" style="margin-bottom:0;">
                    <label for="donation_pix_key">Chave PIX</label>
                    <input id="donation_pix_key" name="donation_pix_key" value="<?= e($readSetting('donation_pix_key')) ?>">
                </div>

                <div class="card" style="margin-bottom:0;">
                    <label for="donation_paypal_url">Link de pagamento (PayPal ou similar)</label>
                    <input id="donation_paypal_url" name="donation_paypal_url" value="<?= e($readSetting('donation_paypal_url')) ?>">
                </div>

                <div class="card" style="margin-bottom:0;">
                    <label for="donation_qr_image">Imagem QR (URL ou caminho relativo)</label>
